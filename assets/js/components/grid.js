    ...
    UI.component('grid', {
        ...
        boot:  function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('[data-sb-grid]', context).each(function(){

                    var ele = UI.$(this);

                    if(!ele.data('grid')) {
                        UI.grid(ele, UI.Utils.options(ele.attr('data-sb-grid')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            if (this.options.controls) {
                ...
                // filter
                this.controls.on('click', '[data-sb-filter]', function(e){
                    e.preventDefault();
                    $this.filter(UI.$(this).attr('data-sb-filter'));
                });

                // sort
                this.controls.on('click', '[data-sb-sort]', function(e){
                    e.preventDefault();
                    var cmd = UI.$(this).attr('data-sb-sort').split(':');
        ...
        filter: function(filter) {
            ...
            if (this.controls && this.controls.length) {
                this.controls.find('[data-sb-filter]').removeClass('sb-active').filter('[data-sb-filter="'+filter+'"]').addClass('sb-active');
            }
        },
        ...
        sort: function(by, order){
            ...
            if (this.controls && this.controls.length) {
                this.controls.find('[data-sb-sort]').removeClass('sb-active').filter('[data-sb-sort="'+by+':'+(order == -1 ? 'desc':'asc')+'"]').addClass('sb-active');
            }
        }
    });

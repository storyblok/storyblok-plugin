...
(function(addon) {
    ...
    UI.component('htmleditor', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('textarea[data-sb-htmleditor]', context).each(function() {

                    var editor = UI.$(this);

                    if (!editor.data('htmleditor')) {
                        UI.htmleditor(editor, UI.Utils.options(editor.attr('data-sb-htmleditor')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.content    = this.htmleditor.find('.sb-htmleditor-content');
            this.toolbar    = this.htmleditor.find('.sb-htmleditor-toolbar');
            this.preview    = this.htmleditor.find('.sb-htmleditor-preview').children().eq(0);
            this.code       = this.htmleditor.find('.sb-htmleditor-code');
            ...
            if (this.options.iframe) {
                ...
                this.iframe = UI.$('<iframe class="sb-htmleditor-iframe" frameborder="0" scrolling="auto" height="100" width="100%"></iframe>');
            ...
            this.htmleditor.on('click', '.sb-htmleditor-button-code, .sb-htmleditor-button-preview', function(e) {

                e.preventDefault();

                if ($this.htmleditor.attr('data-mode') == 'tab') {

                    $this.htmleditor.find('.sb-htmleditor-button-code, .sb-htmleditor-button-preview').removeClass('sb-active').filter(this).addClass('sb-active');

                    $this.activetab = UI.$(this).hasClass('sb-htmleditor-button-code') ? 'code' : 'preview';
            ...
            this.element.attr('data-sb-check-display', 1).on('display.uk.check', function(e) {
        ...
        _buildtoolbar: function() {
            ...
            this.options.toolbar.forEach(function(button) {
                ...
                bar.push('<li><a data-htmleditor-button="'+button+'" title="'+title+'" data-sb-tooltip>'+$this.buttons[button].label+'</a></li>');
            });
        ...
        fit: function() {
            ...
            if (mode == 'tab') {
                ...
                this.htmleditor.find('.sb-htmleditor-button-code, .sb-htmleditor-button-preview').removeClass('sb-active')
                    .filter(this.activetab == 'code' ? '.sb-htmleditor-button-code' : '.sb-htmleditor-button-preview')
                    .addClass('sb-active');
            }
    ...
    UI.components.htmleditor.template = [
        '<div class="sb-htmleditor sb-clearfix" data-mode="split">',
            '<div class="sb-htmleditor-navbar">',
                '<ul class="sb-htmleditor-navbar-nav sb-htmleditor-toolbar"></ul>',
                '<div class="sb-htmleditor-navbar-flip">',
                    '<ul class="sb-htmleditor-navbar-nav">',
                        '<li class="sb-htmleditor-button-code"><a>{:lblCodeview}</a></li>',
                        '<li class="sb-htmleditor-button-preview"><a>{:lblPreview}</a></li>',
                        '<li><a data-htmleditor-button="fullscreen"><i class="sb-icon-expand"></i></a></li>',
                    '</ul>',
                '</div>',
            '</div>',
            '<div class="sb-htmleditor-content">',
                '<div class="sb-htmleditor-code"></div>',
                '<div class="sb-htmleditor-preview"><div></div></div>',
            '</div>',
        '</div>'
    ].join('');
    ...
    UI.plugin('htmleditor', 'base', {
        ...
        init: function(editor) {
            ...
            editor.addButtons({
                ...
                fullscreen: {
                    title  : 'Fullscreen',
                    label  : '<i class="sb-icon-expand"></i>'
                },
                bold : {
                    title  : 'Bold',
                    label  : '<i class="sb-icon-bold"></i>'
                },
                italic : {
                    title  : 'Italic',
                    label  : '<i class="sb-icon-italic"></i>'
                },
                strike : {
                    title  : 'Strikethrough',
                    label  : '<i class="sb-icon-strikethrough"></i>'
                },
                blockquote : {
                    title  : 'Blockquote',
                    label  : '<i class="sb-icon-quote-right"></i>'
                },
                link : {
                    title  : 'Link',
                    label  : '<i class="sb-icon-link"></i>'
                },
                image : {
                    title  : 'Image',
                    label  : '<i class="sb-icon-picture-o"></i>'
                },
                listUl : {
                    title  : 'Unordered List',
                    label  : '<i class="sb-icon-list-ul"></i>'
                },
                listOl : {
                    title  : 'Ordered List',
                    label  : '<i class="sb-icon-list-ol"></i>'
                }
            ...
            editor.htmleditor.on('click', 'a[data-htmleditor-button="fullscreen"]', function() {
                ...
                editor.htmleditor.toggleClass('sb-htmleditor-fullscreen');

                var wrap = editor.editor.getWrapperElement();

                if (editor.htmleditor.hasClass('sb-htmleditor-fullscreen')) {
        ...
        init: function(editor) {
            ...
            UI.$.extend(editor, {
                ...
                disableMarkdown: function() {
                    ...
                    this.htmleditor.find('.sb-htmleditor-button-code a').html(this.options.lblCodeview);
                    this.render();
                }
            ...
            function enableMarkdown() {
                ...
                editor.htmleditor.find('.sb-htmleditor-button-code a').html(editor.options.lblMarkedview);
            }

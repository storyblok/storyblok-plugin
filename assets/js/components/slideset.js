    ...
    UI.component('slideset', {
        ...
        boot: function() {
            ...
            UI.ready(function(context) {
                ...
                UI.$('[data-sb-slideset]', context).each(function(){

                    var ele = UI.$(this);

                    if(!ele.data('slideset')) {
                        UI.slideset(ele, UI.Utils.options(ele.attr('data-sb-slideset')));
                    }
                });
            });
        },
        ...
        init: function() {
            ...
            this.list      = this.element.find('.sb-slideset');
            this.nav       = this.element.find('.sb-slideset-nav');
            ...
            $this.list.addClass('sb-grid-width-1-'+$this.options.default);
            ...
            ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                ...
                $this.list.addClass('sb-grid-width-'+bp+'-1-'+$this.options[bp]);
            });

            this.on('click.uk.slideset', '[data-sb-slideset-item]', function(e) {
                ...
                var set = UI.$(this).attr('data-sb-slideset-item');
            ...
            this.controls.on('click.uk.slideset', '[data-sb-filter]', function(e) {

                var ele = UI.$(this);

                if (ele.parent().hasClass('sb-slideset')) {
                    return;
                }

                e.preventDefault();

                if ($this.animating || $this.currentFilter == ele.attr('data-sb-filter')) {
                    return;
                }

                $this.updateFilter(ele.attr('data-sb-filter'));
            ...
            UI.domObserve(this.list, function(e) {
                if ($this.list.children(':visible:not(.sb-active)').length) {
        ...
        update: function(animate, force) {
            ...
            if (this.nav.length && this.nav.empty()) {
                ...
                for (i=0;i<this.sets.length;i++) {
                    this.nav.append('<li data-sb-slideset-item="'+i+'"><a></a></li>');
                }

                this.nav[this.nav.children().length==1 ? 'addClass':'removeClass']('sb-invisible');
            }
        ...
        updateFilter: function(currentfilter) {
            ...
            this.controls.find('[data-sb-filter]').each(function(){

                filter = UI.$(this);

                if (!filter.parent().hasClass('sb-slideset')) {

                    if (filter.attr('data-sb-filter') == $this.currentFilter) {
                        filter.addClass('sb-active');
                    } else {
                        filter.removeClass('sb-active');
                    }
                }
            });
        },
        ...
        getVisibleOnCurrenBreakpoint: function() {
            ...
            var breakpoint  = null,
                ...
                ['xlarge', 'large', 'medium', 'small'].forEach(function(bp) {
                    ...
                    tmp.attr('class', 'sb-grid-width-'+bp+'-1-2').width();
        ...
        getItems: function() {
            ...
            if (this.currentFilter) {
                ...
                this.children.each(function(index){
                    ...
                    var ele = UI.$(this), f = ele.attr('data-sb-filter'), infilter = filter.length ? false : true;
        ...
        show: function(setIndex, noanimate, dir) {
            ...
            if (this.nav.length) {
                this.nav.children().removeClass('sb-active').eq(setIndex).addClass('sb-active');
            }
            ...
            animation.apply($this, [current, next, dir]).then(function(){
                ...
                $this.children.hide().removeClass('sb-active');
                next.addClass('sb-active').css({'display': '', 'opacity':''});
    ...
    function coreAnimation(cls, current, next, dir) {
        ...
        release = function() {
            ...
            if (current && current.length) {
                current.hide().removeClass(clsOut+' sb-animation-reverse').css({'opacity':'', 'animation-delay': '', 'animation':''});
            }
            ...
            for (i=0;i<current.length;i++) {
                ...
                (function (index, ele){
                    ...
                    setTimeout(function(){
                        ...
                        }).addClass(clsOut+' sb-animation-reverse');
